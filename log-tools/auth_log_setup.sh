...
# -----------------
# Purpose:
#   Creates or refreshes an example /var/log/auth_log.log file with a mix
#   of normal and suspicious entries, mainly for testing log analysis tools.
#
# Usage:
#   sudo ./auth_log_setup.sh
#
# Notes:
#   - Writes 60 lines total. The file gets overwritten each time.
...
# Add 20 pairs of normal+suspicious = 40 lines
for i in {1..20}; do
  echo "${normal_entries[$((RANDOM % ${#normal_entries[@]}))]}" | sudo tee -a "$LOG_FILE" >/dev/null
  echo "${suspicious_entries[$((RANDOM % ${#suspicious_entries[@]}))]}" | sudo tee -a "$LOG_FILE" >/dev/null
done
